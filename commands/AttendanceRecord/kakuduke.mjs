...
export async function execute(interaction) {
  ...
  try {
    ...
    if (records.length === 0) {
      await interaction.reply("過去1週間の出席記録が見つかりませんでした。");
      return;
    }
    ...
    const userAverages = records.reduce((acc, record) => {
      ...
      const entryTime = new Date(`1970-01-01T${record.entryTime}Z`).getTime();  // エポックからのミリ秒に変換
    ...
    // 平均出席時刻を計算
    const sortedUsers = Object.values(userAverages).map(user => {
      return {
        username: user.username,
        averageEntryTime: new Date(user.totalEntryTime / user.count).toISOString().substr(11, 8)  // 平均時刻をHH:mm:ss形式に変換
      };
    }).sort((a, b) => a.averageEntryTime.localeCompare(b.averageEntryTime));  // 早い順にソート

    if (sortedUsers.length > 0) {
      const guild = interaction.guild;
      const highestRole = guild.roles.cache.find(role => role.name === 'The Highest T');
      const lowestRole = guild.roles.cache.find(role => role.name === '激 Low');
      const highTRole = guild.roles.cache.find(role => role.name === 'High T');
      ...
      const highestMember = await guild.members.fetch({ query: highestUser.username, limit: 1 }).then(members => members.first());
      ...
      const lowestMember = await guild.members.fetch({ query: lowestUser.username, limit: 1 }).then(members => members.first());
      ...
      for (const user of sortedUsers) {
        const averageEntryTimeInMs = new Date(`1970-01-01T${user.averageEntryTime}Z`).getTime();
        if (averageEntryTimeInMs < highTThreshold) {
          const member = await guild.members.fetch({ query: user.username, limit: 1 }).then(members => members.first());
          if (member && highTRole) {
      ...
      const embed = new EmbedBuilder()
        .setTitle("格付けのお時間")
        .setColor(0xFFD700)
        .setDescription("過去1週間の平均出席時刻に基づく格付け。1位には 'The Highest T'、最下位には '激 Low'、7:15より早いユーザーには 'High T'が付与されます。");
      ...
      sortedUsers.forEach((user, index) => {
        ...
        embed.addFields({ name: rankMessage, value: `${user.username} - 平均出席時刻: ${user.averageEntryTime}` });
      });
