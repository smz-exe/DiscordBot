...
export const data = new SlashCommandBuilder()
  .setName("attend_world")
  .setDescription("タイムゾーンを指定して出席を記録します。")
  .addStringOption(option =>
    option.setName('timezone')
      .setDescription('タイムゾーンを選択してください')
      .setRequired(true)
      .addChoices(
        { name: 'Tokyo', value: 'Asia/Tokyo' },
        { name: 'California', value: 'America/Los_Angeles' },
        { name: 'Paris/Germany', value: 'Europe/Paris' }
      ));
...
export async function execute(interaction) {
  ...
  const localTime = new Date(formatInTimeZone(now, timeZone, 'yyyy-MM-dd HH:mm:ss'));
  ...
  try {
    ...
    // 新しい出席データを保存
    const attendance = await Attendance.create({ date: today, entryTime: currentTime, UserId: user.id });
    ...
    const embed = new EmbedBuilder()
      .setTitle("出席を確認")
      .setColor(0x00AE86)
      .setDescription(`${username} が ${currentTime} に出席。 ${message}`);
