...
export async function execute(interaction) {
  ...
  try {
    ...
    if (records.length === 0) {
      await interaction.reply("No attendance records found for the past week.");
      return;
    }
    ...
    const labels = Array.from(new Set(records.map(record => record.date))).sort();
    ...
    const datasets = Object.keys(userMap).map(username => {
      const data = labels.map(date => {
        ...
        if (time) {
          const yValue = parseFloat(time.split(':')[0]) + parseFloat(time.split(':')[1]) / 60;
          console.log(`Data point for ${username}:`, { x: date, y: yValue });
          return yValue;
        } else {
    ...
    // 画像をDiscordに送信
    const attachment = new AttachmentBuilder(imageUrl, { name: 'attendance_graph.png' });
    await interaction.reply({ files: [attachment] });
  } catch (error) {
    console.error('Error generating graph:', error);
    await interaction.reply("An error occurred while generating the graph.");
  }
}