...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) => {
    option.setName("section").setDescription("セクション名").setRequired(true);
  ...
  .addIntegerOption((option) =>
    option.setName("num_questions").setDescription("問題数").setRequired(true)
  )
  .addIntegerOption((option) =>
    option
...
export async function execute(interaction) {
  ...
  try {
          ...
          <div class="question-container">
            ${selectedQuestions.map((item, index) => `
              <div class="question">
                ...
                <div class="question-text">${format === 'enToJp' ? item.english : item.japanese}</div>
        ...
        <body>
          ...
          <div class="question-container">
            ${selectedQuestions.map((item, index) => `
              <div class="question">
                <div class="question-number">${index + 1}.</div>
                <div class="question-text">${format === 'enToJp' ? `${item.english} - ${item.japanese}` : `${item.japanese} - ${item.english}`}</div>
                </div>
            `).join('')}
          </div>
        </body>
    ...
    const testAttachment = new AttachmentBuilder(testPdfBuffer, {
    ...
    setTimeout(async () => {
      ...
      const answerAttachment = new AttachmentBuilder(answerPdfBuffer, {
