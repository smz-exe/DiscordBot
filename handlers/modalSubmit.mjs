...
export default async function handleModalSubmit(interaction) {
    ...
    try {
      ...
      if (!user) {
        const embed = new EmbedBuilder()
          .setTitle("出席記録失敗")
          .setColor(0xFF0000)
          .setDescription("最初に /register を使って登録する必要があります。");
      ...
      if (existingAttendance) {
        ...
        const embed = new EmbedBuilder()
          .setTitle("出席記録更新")
          .setColor(0x00AE86)
          .setDescription(`出席記録が更新されました。${date} の ${time} に変更されました。`);

        await interaction.reply({ embeds: [embed], ephemeral: true });
      } else {
        const attendance = await Attendance.create({ date: date, entryTime: time, UserId: user.id });
        ...
        const embed = new EmbedBuilder()
          .setTitle("手動出席を確認")
          .setColor(0x00AE86)
          .setDescription(`ユーザー名 ${username} が ${date} の ${time} に出席しました。`);
  ...
  if (interaction.customId === 'testScoreModal') {
    ...
    const score = parseInt(interaction.fields.getTextInputValue('score'), 10);
    const totalQuestions = parseInt(interaction.fields.getTextInputValue('total_questions'), 10);
    ...
    if (isNaN(score) || isNaN(totalQuestions) || totalQuestions <= 0) {
      const embed = new EmbedBuilder()
        .setTitle("記録失敗")
        .setColor(0xFF0000)
        .setDescription("得点と問題数は有効な数値でなければなりません。");
    ...
    try {
      ...
      if (existingTestScore) {
        await existingTestScore.update({ score, totalQuestions, normalizedScore });
        const embed = new EmbedBuilder()
          .setTitle("テスト結果更新")
          .setColor(0x00AE86)
          .setDescription(`テスト結果が更新されました。${formattedDate} のスコアは ${score}/${totalQuestions} (${normalizedScore.toFixed(2)}%) です。`);

        await interaction.followUp({ embeds: [embed] });
      } else {
        const testScore = await TestScore.create({ userId: user.id, date: formattedDate, score, totalQuestions, normalizedScore });
        ...
        const embed = new EmbedBuilder()
          .setTitle("テスト結果を記録")
          .setColor(0x00AE86)
          .setDescription(`ユーザー名 ${username} が ${formattedDate} に ${score}/${totalQuestions} の得点を記録しました。\n(100点満点換算: ${normalizedScore.toFixed(2)}点)`);
